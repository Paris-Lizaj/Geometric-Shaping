...
Power_dBm=[-9:1:1];   % This is the input power that you wan't to use (dBm)
%Power_dBm=[-6]
mW1=1e-3;
Nt=2^19; %%%%%%%%%% number of samples
Spans=[10:1:20]; %that means that we take into account Spans*L total transmission length
% Spans=[10]
	
Power_Watt=mW1*10.^(Power_dBm/10)/10;     % This is the input power that you wan't to use (W)
Power_Watt2=1*Power_Watt;   % for wdm channels . Se to 0*Power_Watt for single channel transmission   


num_of_total_channels =9;
M=16; % M-QAM modulation format
M2=16; % M-QAM modulation format

downDBP=4;
dt=4e-11/32; %%%%% dt: sampling time for 32
% dt=4e-11/64; %%%%% dt: sampling time
...
df=50e9; % wavelength grid = 50 GHz = 0.4 nmout
...
b2=-21e-27; %%%%%%%%%%% second order dispersion in s^2/m 
...
span_RRC = 20;     % Filter span in symbols
sps=4;            % Samples per symbol
b = rcosdesign(rolloff, span_RRC, sps);

%% LOOP Input Power soans and geoshape
for igrey=1:length(GreyCode)
for ispan=1:length(Spans)
for iii=1:length(Power_dBm) % From -5 dBm to 3 dBm  with step 0.5 dB 



rng('shuffle');
for chan=1:2*num_of_total_channels
    data{chan}=randi([0,M-1],1,symbols); 
    mod_data{chan}=genqammod(data{chan},GreyCode{igrey}); %modulated data
    mod_data_rrc{chan} = upfirdn(mod_data{chan}, b, sps);
%     mod_data_rrc=mod_data;
 
    meanP{chan}=mean(abs(mod_data_rrc{chan}).^2);
...
for kk=1:length(cell2mat(mod_data_rrc(chan)))
%     Resampled{chan}(samples_symbol*(kk-1)+1:samples_symbol*kk)=mod_data{chan}(kk);
    Resampled{chan}(round(samples_symbol/sps)*(kk-1)+1:round(samples_symbol)/sps*kk)=mod_data_rrc{chan}(kk);
end
...
Power_W_central=mean(abs(A{1}(Diff{1}/2:length(Resampled{1})+Diff{1}/2-1)).^2) % print the actual mean power after modulation, this is the power you should include in your x axis. 
Power_dBm_central=10*log10(Power_W_central/mW1);
...
%% now checking only Ax1 Ay1 / Uncheck after  
Ax=A{1}+...
...
%% Transmission --- 
tic       
% [Ax, Ay]= span_mex2_23_8388608(Spans, Nz, Ax, g, dz, Ay, Linear, G, h, fo, nsp, dt);
...
for kk=1:length(mod_data_rrc{1})
out_data_x_rcc(kk)=out_complex_x(round(Diff_x/2+samples_symbol2/sps*(kk-1)+samples_symbol2/sps/2-0*t_cor_x));
out_data_y_rcc(kk)=out_complex_y(round(Diff_y/2+samples_symbol2/sps*(kk-1)+samples_symbol2/sps/2-0*t_cor_y));
% out_data1(kk)=out_complex1(Diff/2+samples_symbol*(kk-1)+samples_symbol/2-t_cor1);
% out_data2(kk)=out_complex2(Diff/2+samples_symbol*(kk-1)+samples_symbol/2-t_cor2);
end
%%

out_data_x_rcc=sqrt(meanP{1})*out_data_x_rcc/sqrt(mean(abs(out_data_x_rcc).^2));
out_data_y_rcc=sqrt(meanP{2})*out_data_y_rcc/sqrt(mean(abs(out_data_y_rcc).^2));

out_data_x = upfirdn(out_data_x_rcc, b, 1, sps);
out_data_y = upfirdn(out_data_y_rcc, b, 1, sps);
out_data_x(1:span_RRC)=[];
out_data_x((end-span_RRC+1):end)=[];
out_data_y(1:span_RRC)=[];
out_data_y((end-span_RRC+1):end)=[];
% out_data1=sqrt(meanP1)*out_data1/sqrt(mean(abs(out_data1).^2));
% out_data2=sqrt(meanP2)*out_data2/sqrt(mean(abs(out_data2).^2));
...
% out_data_shift1=out_data1*exp(-k*1i*2*pi/360);
% out_data_shift2=out_data2*exp(-k*1i*2*pi/360);
% scatterplot(out_data_shift)

Rec_data_x=genqamdemod(out_data_shift_x, GreyCode{igrey});
Rec_data_y=genqamdemod(out_data_shift_y, GreyCode{igrey});
% Rec_data1=qamdemod(out_data_shift1, M);
% Rec_data2=qamdemod(out_data_shift2, M);
...
minimum_x = min(BER_x);
Ix=find(BER_x==minimum_x);
minBERx=BER_x(Ix(1))

minimum_y = min(BER_y);
Iy=find(BER_y==minimum_y);
minBERy=BER_y(Iy(1))


% scatterplot(out_data_x*exp(-Ix(1)*1i*2*pi/360))
% scatterplot(out_data_y*exp(-Iy(1)*1i*2*pi/360))
...
dlmwrite('PowerBERRRC.txt',writtenData, '-append');

X_x_real=real(out_data_x*exp(-Ix(1)*1i*2*pi/360))';
X_x_imag=imag(out_data_x*exp(-Ix(1)*1i*2*pi/360))';

X_y_real=real(out_data_y*exp(-Iy(1)*1i*2*pi/360))';
X_y_imag=imag(out_data_y*exp(-Iy(1)*1i*2*pi/360))';
...
% BERx_noRRC=csvread('BERx_noRRC.txt');
% BERx_RRC8=csvread('BERx_RRC10span_8samples.txt'); 
% BERx_RRC4=csvread('BERx_RRC10span.txt'); 
% figure;
% plot(BERx_noRRC);
% hold on;
% plot(BERx_RRC8);
% hold on;
% plot(BERx_RRC4);
% legend('BERx no RRC','BERx RRC 8samples/symbol 10spans','BERx RRC 4samples/symbol 10spans')
